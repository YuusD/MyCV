...
class Apriltag(object):
    ...
    def detect(self,frame):
        ...
        """
        2 adaptive thresholding or  canny 自适应阈值或canny
        """
...
#            img = cv2.GaussianBlur(img, (7, 7), self._quad_sigma)
        ...
        """
        3 find contours 查找轮廓
        """
        if (self._thresholding == 'canny'):
            imgC, contours, hierarchy = cv2.findContours(img, cv2.RETR_CCOMP, cv2.CHAIN_APPROX_SIMPLE)
        else:
            imgC, contours, hierarchy = cv2.findContours(img, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
        ##################################
        time_end = time.time()
        if self._debug:
            print(len(contours))
        ...
        """
        4 compute convex hulls and find maximum inscribed quadrilaterals 计算凸壳并求最大内接四边形
        """
        ...
        ##################################
        """
        5 decode and get detections 解码并检测
        """
