'''
StereoMoulds模块主要提供双目立体校正、视差计算和深度计算功能
立体校正：rectifyImage
视差计算：disparity_SGBM  SGBM算法-匹配代价部分用的是BT代价（原图+梯度图）
深度计算：cv2.reprojectImageTo3D
'''
import cv2
import numpy as np
import time
...
#重写 cv2.reprojectImageTo3D函数
def stereoto3d(pointls,pointrs, Q):
    ...
    for i in len(pointls):
        ...
        pw=1./((xlm-xrm)*Tx+E)
        pointss[i][0]=(xlm+cx)*pw
        pointss[i][1]=(ylm+cy)*pw
        pointss[i][2]=f*pw
...
if __name__ == '__main__':
    ...
    iml = cv2.imread('im0.png')   # 左图
    imr = cv2.imread('im1.png')   # 右图
